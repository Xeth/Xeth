...
var appStatusMessages = {synchronizing:'Synchronizing with network', connecting:'Connecting to network', scanning:"Scanning transactions", ok:''};
...
function init() 
{
    ...
    event.Ready.connect(this, pageStart);
}
...
function initProgress(){
    event.SyncProgress.connect(this, syncProgressLoader);
    appStatusMessage("synchronizing");
    syncProgressLoader(progress.getSync());
}

function syncProgressLoader(percent){
    if(percent>99.9){
        event.SyncProgress.disconnect(this, syncProgressLoader);
        appStatusMessage("scanning");
        event.ScanProgress.connect(this, scanProgressLoader);
        scanProgressLoader(progress.getScan());
    }else{
        appStatusLoader(percent);
        appStatusLoaderText(percent.toFixed(2)+"%");
    }
}

function scanProgressLoader(percent){
    appStatusLoader(percent);
    appStatusLoaderText(percent.toFixed(2)+"%");
    if(percent>99.9){
        appStatusIcon_updated(true);
        appStatusMessage("ok");
        event.ScanProgress.disconnect(this, scanProgressLoader);
        event.SyncProgress.connect(this, watchSyncProgress);
    }
}

function watchSyncProgress(percent){
    if(percent<95.0){
        appStatusIcon_updated(false);
        initProgress();
    }
}
...
function appStatusLoaderText(text){
    $('.footer .status .loader .txt').text(text);
}
